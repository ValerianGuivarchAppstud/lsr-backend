...
export class MjSheetVM {
  ...
  static from(p: { mj: Mj; characters: Character[]; rollList: Roll[] }): MjSheetVM {
    return new MjSheetVM({
      characters: p.characters.map((character) => CharacterVM.from({ character: character })),
      pjNames: p.mj.pjNames,
      pnjNames: p.mj.pnjNames,
      tempoNames: p.mj.tempoNames,
      playersName: p.mj.playersName,
